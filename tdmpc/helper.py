"""
Ported from the official TD-MPC implementation (Hansen, Wang, Su 2022):
https://github.com/nicklashansen/tdmpc (src/algorithm/helper.py)

Kept as close to the original as possible; adapted only for this repo's
conventions (obs already frame-stacked upstream by tdmpc/env_wrapper.py,
so Episode/ReplayBuffer need no dm_control-specific logic).
"""
import re
import numpy as np
import torch
...
def enc(cfg):
    ...
    if cfg.modality == 'pixels':
        ...
        layers.extend([Flatten(), nn.Linear(int(np.prod(out_shape)), cfg.latent_dim)])
    else:
